...
void			reverse_bytes_384(void *bytes, uint32_t n)
{
	uint32_t	i;
	uint8_t		tmp;
...
int				init_sha384_block(t_input *input)
{
	int			i;
	...
	*((uint64_t *)(S_B->padded_content + P_C_L - 8)) = S_B->content_length * 8;
	reverse_bytes_384(S_B->padded_content + (P_C_L - 8), 8);
...
int				sha384_format_digest(t_ssl *ssl, t_input *input)
{
	if (ssl->q == 1 || ssl->ac == 2)
		ft_printf("%s\n", input->digest);
...
int				sha384_handler(t_ssl *ssl, t_input **input)
{
	...
	while (++i < ssl->current_input)
		init_sha384_block(input[i]);
	i = -1;
	while (++i < ssl->current_input)
		input[i]->digest = sha384(input[i]->b_p->blocks,
									input[i]->b_p->number_blocks);
	i = -1;
	while (++i < ssl->current_input)
		sha384_format_digest(ssl, input[i]);
	return (1);
}
