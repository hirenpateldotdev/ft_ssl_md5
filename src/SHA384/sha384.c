...
static void				init_words(uint8_t *block, uint32_t *sub_block)
{
	...
	V_I = 0;
	while (V_I < 16)
		reverse_bytes_384(sub_block + V_I++, sizeof(int));
	V_I = 16;
	while (V_I < 64)
	{
...
void					sha384_get_hash(uint32_t *hash, uint32_t *sub_block,
										uint32_t i)
{
	uint32_t			*var;
...
uint32_t				*sha384_get_chuck_hash(uint8_t *block, uint32_t *hash)
{
	...
	while (V_I < 64)
	{
		sha384_get_hash(block_hash, sub_block, V_I);
		V_I++;
	}
	free(var);
	return (block_hash);
...
char					*sha384(uint8_t **blocks, int number_blocks)
{
	uint32_t			*block_hash;
	uint32_t			*hash;
	int					i;
	...
	{
		block_hash = sha384_get_chuck_hash(blocks[i], hash);
		x = -1;
		while (++x < 8)
			hash[x] += block_hash[x];
	}
	i = 0;
	while (i < 8)
		reverse_bytes_384(hash + i++, sizeof(int));
	d = sha384_hash((uint8_t*)hash);
	free(hash);
	return (d);
}
